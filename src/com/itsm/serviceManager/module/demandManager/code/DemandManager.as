...
//demand来源
[Bindable]
public var demandSources:ArrayCollection;

//demand作用范围
[Bindable]
public var demandScops:ArrayCollection;
...
//实际作用范围
[Bindable]
public var realDemandScops:ArrayCollection;
...
//流程模型类型
private static const FLOW_TYPE:String="3";
 ...
 * 弹窗下拉菜单初始化
 */
private function initUserApplyInfo():void{
...
//			});
	...
	if(!(demandScops != null))
		BugUtil.handle_server_method("bugManagerAPI","BugManagerAPI","initBugScope",
			[], function(event:ResultEvent):void{
				demandScops = getResultObj(event) as ArrayCollection;
			});
	...
	if(isSelect()){
		if(item != null && item.applyId > 0){
			if(item.applyStatus == 0)
				BugUtil.handle_server_method("bugManagerAPI","BugManagerAPI","getPersonIdByApplyId",
...
private function delApplyProcess(event:CloseEvent):void{
	...
	if(event.detail == TAlert.OK)
		BugUtil.handle_server_method("bugManagerAPI","BugManagerAPI","delUserApplyById",
...
{                     
	...
	formIsEmpty = telephoneInput.text == "" ||
		...
		reasonArea.text == "" || 
		demandSystem.sText == "" || 
		demandSystem.sText == null || 
		demandModule.sText == "" || 
		demandModule.sText == null;
...
protected function saveBtn_clickHandler(event:MouseEvent):void
{
	if(validate()){
		...
		if(this.currentState == "add"){
			...
			//			BugUtil.handle_server_method("bugManagerAPI","BugManagerAPI","addBugUserApply",
			//				[JSON.stringify(ac.source)], saveBtn_clickHandle);
			BugUtil.handle_server_method("demandManagerAPI","DemandManagerAPI","addDemandUserApply",
				[appCore.jsonEncoder.encode(storeSimpleValue2userApply())], saveBtn_clickHandle);
			return;
		}
		//updateDemandUerApply
		BugUtil.handle_server_method("bugManagerAPI","BugManagerAPI","updateBugUerApplyAdd",
			[appCore.jsonEncoder.encode(storeSimpleValue2userApply())], saveBtn_clickHandle);	
	}
}
...
private function storeSimpleValue2userApply():Object{
	...
	//对象处理:Demand来源,Demand模块,Demand系统,Demand影响范围,demand重要程度
	var origin:ConstDetailVO = new ConstDetailVO();
	if(demandSource.selectedItem != null)
		origin.id = demandSource.selectedItem.id;
	obj.applyOrigin = origin;
	
	var module:SystemModuleVO = new SystemModuleVO();
	module.moduleID = demandModule.selectObj.hasOwnProperty("moduleID")
		?demandModule.selectObj.moduleID:demandModule.sText != "" 
		&& demandModule.sText != null
		?userApply.sysModule.moduleID:"-1";
	obj.sysModule = module;
	
	var system:SupportSystemVO = new SupportSystemVO();
	system.systemID = demandSystem.selectObj.hasOwnProperty("systemID")
		?demandSystem.selectObj.systemID:demandSystem.sText != "" 
		&& demandSystem.sText != null
...
//点击修改界面的demand来源下拉列表时调用==========demand来源
protected function demandSource_clickHandler(event:MouseEvent):void
{
	
	//先查看demandSources是否为null，为null再向服务器请求数据
	if(demandSources == null){
		BugUtil.handle_server_method("bugManagerAPI","BugManagerAPI","initBugSource",
			[], demandSource_clickHandle);
	}else{
		demandSource.dataProvider = demandSources;
	}
	//由于界面值有修改,so--将呈报按钮置灰,保存按钮激活
	enablePutBtn = false;
}
...
private function demandSource_clickHandle(event:ResultEvent):void{
	demandSources = getResultObj(event) as ArrayCollection;
	demandSource.dataProvider = demandSources;
}
...
protected function demandRange_clickHandler(event:MouseEvent):void
{
	...
	if(demandScops == null){
		...
		demandRange.dataProvider = demandScops;
	}
	//由于界面值有修改,so--将呈报按钮置灰,保存按钮激活
	enablePutBtn = false;
}
...
//protected function demandSystem_clickHandler(event:MouseEvent):void
//{
//	//系统下拉被点击标志
...
//	if(systems == null){
//		BugUtil.handle_server_method("bugManagerAPI","BugManagerAPI","initSupportSystems",
//			[], demandSystem_clickHandle);
//	}else{
//		var sysid:uint = demandSystem.selectedItem.systemID;
//		demandSystem.dataProvider = systems;
//		if(demandSystem.selectedIndex == 0){
//			modules = systems.getItemAt(0).systemModules as ArrayCollection;
//			demandModule.dataProvider = modules;
//		}else{
...
//				flag = true;
//			if(flag)
//				BugUtil.handle_server_method("bugManagerAPI","BugManagerAPI","initSystemModules",
//					[sysid], demandModule_clickHandle);
//		}
//			
//	}
//}
...
//private function demandSystem_clickHandle(event:ResultEvent):void{
//	systems = getResultObj(event) as ArrayCollection;
//	modules = systems.getItemAt(0).systemModules as ArrayCollection;
//	
//	demandSystem.dataProvider = systems;
...
//protected function demandModule_clickHandler(event:MouseEvent):void
//{
//	var sysid:uint;
//	if(userApply == null || isSystemBeClicked){
//		sysid = demandSystem.selectedItem != null 
//			? demandSystem.selectedItem.systemID : -1;
//	}else
...
//private function demandModule_clickHandle(event:ResultEvent):void{
//	var obj:Object = getResultObj(event);
//	
...
//protected function demandRealRange_clickHandler(event:MouseEvent):void
//{
//	//先查看realDemandScops是否为null，为null再向服务器请求数据
//	if(realDemandScops == null){
//		BugUtil.handle_server_method("bugManagerAPI","BugManagerAPI","initBugScope",
//			[], demandRealRange_clickHandle);
//	}else{
//		demandRealRange.dataProvider = realDemandScops;
//	}
//}
...
//private function demandRealRange_clickHandle(event:ResultEvent):void{
//	realDemandScops = getResultObj(event) as ArrayCollection;
//	
//	demandRealRange.dataProvider = realDemandScops;
//}

/**系统更改*/
protected function demandSystem_selectChangeHandler(event:Event):void
{
	demandModule.selectObj = "";
	demandModule.sText = "";
	systemID = demandSystem.selectObj.hasOwnProperty("systemID")
		?demandSystem.selectObj.systemID:(demandSystem.sText != "" 
			&& demandSystem.sText != null)
...
//点击首页的新增按钮后，会从数据库中查询出demand来源、demand作用范围、所属系统、所属业务模块、实际作用范围的列表
...
public function addBtn_clickHandle(event:ResultEvent):void{
	...
	//分类存储
	//来源
	demandSources = list.getItemAt(1) as ArrayCollection;
	//作用范围
	demandScops = list.getItemAt(0) as ArrayCollection;
//	//系统
//	systems = list.getItemAt(3) as ArrayCollection;
//	//第一个系统的业务模块
//	modules = systems.getItemAt(0).systemModules as ArrayCollection;
	//实际作用范围
	realDemandScops = demandScops;
	//重要程度
...
////所属系统更改时调用
//protected function demandSystem_changeHandler(event:IndexChangeEvent):void
//{
//	var sysID:int = demandSystem.selectedItem.systemID;
//	BugUtil.handle_server_method("bugManagerAPI","BugManagerAPI","initSystemModules",
//		[sysID], demandSystem_changeHandle);
...
//}
//
//private function demandSystem_changeHandle(event:ResultEvent):void{
//	//由于界面值有修改,so--将呈报按钮置灰,保存按钮激活
//	enablePutBtn = false;
...
//	
//	
//	//对象处理:demand来源,demand模块,demand系统,demand影响范围,demand实际影响范围,demand重要程度,请求单状态
//	var origin:ConstDetailVO = new ConstDetailVO();
//	if(demandSource.selectedItem != null)
//		origin.id = demandSource.selectedItem.id;
//	apply.applyOrigin = origin;
//	
//	var module:SystemModuleVO = new SystemModuleVO();
//	if(demandModule.selectedItem != null)
//		module.moduleID = demandModule.selectedItem.moduleID;
//	apply.sysModule = module;
//	
//	var system:SupportSystemVO = new SupportSystemVO();
//	if(demandSystem.selectedItem != null)
//		system.systemID = demandSystem.selectedItem.systemID;
//	apply.belongSystem = system;
//	
//	var range:ConstDetailVO = new ConstDetailVO();
//	if(demandRange.selectedItem != null)
...
////	if(demandRealRange.selectedItem != null)
////		realRange.id = demandRealRange.selectedItem.id;
...
private function createCode():void{
	...
	//得到所有的bug
	BugUtil.handle_server_method("bugManagerAPI","BugManagerAPI","findAllApplyCodes",
		[], createCodeHandle);
...
//	//demand来源
//	demandSource.dataProvider = getArrayListFromObject(userApply.applyOrigin);
//	demandSource.selectedIndex = 0;
//	//重要程度
//	demandUrgent.dataProvider = getArrayListFromObject(userApply.urgent);
//	demandUrgent.selectedIndex = 0;
//	//影响范围
//	demandRange.dataProvider = getArrayListFromObject(userApply.range);
//	demandRange.selectedIndex = 0;
//	//所属系统
//	demandSystem.dataProvider = getArrayListFromObject(userApply.belongSystem);
//	demandSystem.selectedIndex = 0;
//	//所属业务模块
//	demandModule.dataProvider = getArrayListFromObject(userApply.sysModule);
//	demandModule.selectedIndex = 0;
//	//实际影响范围
//	demandRealRange.dataProvider = getArrayListFromObject(userApply.realRange);
//	demandRealRange.selectedIndex =0;
//	
...
//	applyEndDate.errorString="";
//	
//	demandSource.errorString="";
//	demandUrgent.errorString="";
//	demandRange.errorString="";
//	demandSystem.errorString="";
//	demandModule.errorString="";
//	
...
private function putBtn_clickHandle(event:ResultEvent):void{
	...
	//	BugUtil.handle_server_method("bugManagerAPI","BugManagerAPI","updateBugUerApply",
	//		[str], after_putBtn_clickHandle);
	BugUtil.handle_server_method("bugManagerAPI","BugManagerAPI","updateUerApplyStatus",
