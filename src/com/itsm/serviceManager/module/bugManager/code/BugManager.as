...
//2：代表查询bug
private static const FLAG:int = 2;
...
private function createCode():void{
	...
	//得到所有的bug
	handle_server_method("bugManagerAPI","BugManagerAPI","findAllApplyCodes",
		[], createCodeHandle);
...
private function formatDateTime(item:Object,column:DataGridColumn):String{
	...
	function formateDate(item:Object, flag:int):String{
		...
		if(!dateStr.match(/\d{4}-\d{2}-\d{2}/)){
			...
			month = BugUtil.changeToArabicNum(month);
...
private function resetDropDownList():void{
	//Bug来源
	bugSource.dataProvider = BugUtil.getArrayListFromObject(userApply.applyOrigin);
	bugSource.selectedIndex = 0;
	//重要程度
	bugUrgent.dataProvider = BugUtil.getArrayListFromObject(userApply.urgent);
	bugUrgent.selectedIndex = 0;
	//影响范围
	bugRange.dataProvider = BugUtil.getArrayListFromObject(userApply.range);
...
//	bugModule.selectedIndex = 0;
	//实际影响范围
	bugRealRange.dataProvider = BugUtil.getArrayListFromObject(userApply.realRange);
